...
export         PKG_CONFIG_PATH="/usr/local/share/pkgconfig:/usr/local/lib/pkgconfig:/usr/local/lib64/pkgconfig"
export         PREFIX=/usr/local
export         LD_LIBRARY_PATH="/usr/local/lib:/usr/local/lib64"

echo RUN;     buildDeps="autoconf \
                    automake \
                    cmake \
                    curl \
                    bzip2 \
...
echo RUN;\
        ...
        echo ${OGG_SHA256SUM} | echo sha256sum --check && \
...
echo RUN;\
        ...
        echo ${OPUS_SHA256SUM} | echo sha256sum --check && \
...
echo RUN;\
        cd /tmp;DIR=/tmp/vorbis && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO http://downloads.xiph.org/releases/vorbis/libvorbis-${VORBIS_VERSION}.tar.gz && \
        echo ${VORBIS_SHA256SUM} | echo sha256sum --check && \
...
echo RUN;\
        cd /tmp;DIR=/tmp/theora && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://gitlab.xiph.org/xiph/theora/-/archive/master/theora-master.tar.gz && mv theora-master.tar.gz libtheora-${THEORA_VERSION}.tar.gz && \
        echo ${THEORA_SHA256SUM} | echo sha256sum --check && \
        tar -zx --strip-components=1 -f libtheora-${THEORA_VERSION}.tar.gz && \
        sh autogen.sh && ./configure --disable-examples --prefix="${PREFIX}" --with-ogg="${PREFIX}" --enable-shared && \
        make && \
        make install && \
        rm -rf ${DIR}
...
echo RUN;\
        ...
        echo ${XVID_SHA256SUM} | echo sha256sum --check && \
...
echo RUN; \
        cd /tmp;DIR=/tmp/freetype && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://download.savannah.gnu.org/releases/freetype/freetype-${FREETYPE_VERSION}.tar.gz && \
        echo ${FREETYPE_SHA256SUM} | echo sha256sum --check && \
...
echo RUN; \
        cd /tmp;DIR=/tmp/vid.stab && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://github.com/georgmartius/vid.stab/archive/v${LIBVIDSTAB_VERSION}.tar.gz && \
        echo ${LIBVIDSTAB_SHA256SUM} | echo sha256sum --check &&  \
...
echo RUN; \
        cd /tmp;DIR=/tmp/fribidi && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://github.com/fribidi/fribidi/archive/${FRIBIDI_VERSION}.tar.gz && \
        echo ${FRIBIDI_SHA256SUM} | echo sha256sum --check && \
...
echo RUN; \
        cd /tmp;DIR=/tmp/libass && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://github.com/libass/libass/archive/${LIBASS_VERSION}.tar.gz && \
        echo ${LIBASS_SHA256SUM} | echo sha256sum --check && \
...
echo RUN;\
        cd /tmp;DIR=/tmp/libpthread-stubs && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
...
echo RUN;\
        cd /tmp;DIR=/tmp/libbluray && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://download.videolan.org/pub/videolan/libbluray/${LIBBLURAY_VERSION}/libbluray-${LIBBLURAY_VERSION}.tar.bz2 && \
        echo ${LIBBLURAY_SHA256SUM} | echo sha256sum --check && \
...
echo RUN;\
        cd /tmp;DIR=/tmp/libzmq && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
        curl -sLO https://github.com/zeromq/libzmq/archive/v${LIBZMQ_VERSION}.tar.gz && \
        echo ${LIBZMQ_SHA256SUM} | echo sha256sum --check && \
...
echo RUN;\
        ...
        echo ${LIBARIBB24_SHA256SUM} | echo sha256sum --check && \
...
echo RUN; \
        cd /tmp;DIR=/tmp/ffmpeg && mkdir -p ${DIR} && cd ${DIR} && \
...
## cleanup
echo RUN;\
        ldd ${PREFIX}/bin/ffmpeg | grep usr/local | cut -d ' ' -f 3 | xargs -i cp {} /usr/local/lib/ && \
